...
- Each participant's `color` is display-only, `#RRGGBB`. A client never paints the raw value: it buckets the 24-bit value mod 12 into the frozen identity roster (`vectors/identity/roster.json`) and paints the slot's ground variant. The server assigns the value room-aware (DESIGN.md D28): a member keeps their stable hash color unless its roster slot is already claimed in the room, in which case the emitted value buckets to a perceptually distant free slot (`vectors/identity/room-colors.json`). The client bucketing contract is unchanged.
...
- `playerConnected {userId, displayName, avatarUrl, color, role}` and `playerDisconnected {userId}`. `avatarUrl` is the same opaque nullable field as the participant carries (section 4). `color` is assigned against the room's full membership at emission, the same room-aware rule as the section 4 participants (DESIGN.md D28), so the notice and the board payload agree for one room.
...
- **Reconnect presentation grace.** Chrome for the non-live pair (`resyncing`, `reconnecting`) is withheld until the state has held continuously for 2 s, and it clears the instant the connection recovers; a bounce between the two non-live states shares one window, `connecting` is never gated, and input is never gated by presentation. Rationale: the deploy edge recycles healthy sockets and heals in about 200 ms, and that must pass invisibly. This is presentation only, no wire message; the constant is byte-twinned on all three clients (`RECONNECT_OVERLAY_GRACE_MS = 2000`) (#335, #336, #337).
...
| `GET /games/{id}`                     | member                                  | the game view: solution-stripped puzzle, membership (each member `{userId, role, joinedAt, avatarUrl}`, `avatarUrl` the same opaque nullable field as section 4), session endpoint, the optional `name`, the `inviteCode` (members only, any role: every member joined via it), and the puzzle's display metadata `puzzleTitle` and `puzzleAuthor` (each `string \| null`, read from the `puzzles` row exactly as the list endpoints read them, never from the snapshot; display content for post-game surfaces such as the share card, not solution content, INV-6 untouched; additive per section 14, so an older server omits them and a client reads absence as null) |
| `GET /games/{id}/analysis`            | member, completed game only             | the post-game analysis bundle: `{owners: {[cell]: userId}, momentum: {durationSeconds, samples[40]}, moments: {firstToFall, lastSquare, turningPoint}, sequence: [{cell, atSeconds}], titles: [{userId, title, evidence}], sittings: {count, spans: [{startSeconds, endSeconds}], wallSeconds}}`, the whole completed surface in one fetch. `owners` is the first-correct map (who FIRST placed the correct value in each cell); `momentum` is the room's tempo (a 40-sample peak-normalized ribbon plus the solve's duration); `moments` are three named beats from timing alone (the opening, the closing, the turning point), each nullable; `sequence` is the ordered `{cell, atSeconds}[]`, ascending by `(at, seq)`, each cell and the relative second it first went correct, for the post-game solve replay (another projection of the same trace, no userId, the owner comes from `owners`); `titles` is the solver superlatives, ordered by ladder rank, at most one per solver and one per key (`title` a lowercase-kebab key from the pinned ladder, `evidence` the title's own count or null; a client MUST ignore an unknown key, which is how the ladder grows without client lockstep; empty when fewer than two solvers wrote, regardless of member count; `design/post-game/TITLES.md` pins the stat sheet, the gates, and the ladder. The wire carries the key only; the key's display LABEL (`saboteur` -> `"The saboteur"`) is shared normative ground in `vectors/analysis/title-labels.json`, so the web, iOS, and Android panels and the server-rendered share card all word one title identically, while the evidence/detail line under a label interpolates the solve's own stats and stays client-owned prose). **All trace-projection times are active seconds (DESIGN.md D29):** the projections run over the idle-collapsed event log (`solveTrace(collapseIdle(events), solution)`), where every gap of `SITTING_GAP_MS` (1,800,000 ms) or more between consecutive cell events, seq order, `>=` at the threshold, collapses to exactly zero, so `momentum.durationSeconds`, `momentum`'s axis, `moments`' `atSeconds`/`stallSeconds`/`breakSeconds`, and `sequence[].atSeconds` all measure concatenated active time; field names and shapes are unchanged, the bundle is self-consistent on one axis, and a game with no such gap is the identity mapping, byte-identical to the pre-D29 bundle (`vectors/analysis/sittings.json` pins the partition and the remap). `titles` re-based with the D29 fast-follow: `brokeStall` and the room's stall run on the same collapsed trace (the ribbon's turning point and the ice-breaker card always name the same person), while `span` and `burst` deliberately keep the raw wall-clock events (a collapsed burst window could straddle a seam); `marathoner` (present in every sitting, sitting count as evidence) joins the ladder at rank 8. `sittings` describes the partition itself: `count` sittings; `spans`, one per sitting on the same active axis, contiguous (`spans[k+1].startSeconds == spans[k].endSeconds`, first start 0, last end `momentum.durationSeconds`; a single sitting is one span `[0, durationSeconds]`; a sitting with no trace entry clamps to a zero-width span at the axis edge), for ribbon seam ticks; and `wallSeconds`, the wall-clock trace span the pre-D29 `durationSeconds` reported (equal to `durationSeconds` for a single-sitting game), for flavor copy only, never a competing stat. A client MUST tolerate the `sittings` field's absence (an older bundle) and degrade to rendering without seams. The Archive read model, computed on read from `cell_events` (now also selecting `at`, DESIGN.md section 7, section 9). userIds, cells, and numbers only, no solution content (INV-6); the client already holds the roster from the game view. This MUST stay off the `GET /games` list path, which keeps its cheap `MAX(at)` aggregate; this is a per-game, on-demand read. Gated to completed games (an ongoing game's trace leaks solving progress; an abandoned-game recap is deferred): a game that is not completed is `GAME_NOT_FOUND`, and a non-member is `NOT_PARTICIPANT`, the same gate as the game view. Replaces the unshipped `/attribution` |
| `POST /games/{id}/share`              | member, completed game only             | mint (or return) the game's public share link; returns `{shareUrl, token}`. Idempotent: one active token per game, so a re-POST returns the existing token, not a second live link (the `share_tokens` partial unique index, DESIGN.md section 9). The `token` is an unguessable 256-bit URL-safe secret; `shareUrl` is `{share-origin}/s/{token}`, the `share-origin` the config-driven public host invite links use (`{invite-host}`), else the request origin. Gated exactly as `GET /games/{id}/analysis`: a non-member is `NOT_PARTICIPANT`, a game that is not completed is `GAME_NOT_FOUND`, so a link exists only once the input is frozen (INV-4). The token carries no solution content (INV-6). Additive per section 14 |
| `GET /s/{token}`                      | public                                  | the completion share page: an HTML shell with OpenGraph tags for unfurlers (`og:title` the puzzle title with the card's own fallbacks, `og:image` the card PNG with its `1200x630` dimensions) and a minimal human page (the title, a link to the app, and the replay hero: the mosaic board drawn inline, animating the solve back in `sequence` order — real relative timing compressed onto a fixed loop, each square washing in with its owner's tint or the solo gold ramp, a hold on the finished board, then a dissolve and restart). The replay is pure inline CSS (no script, no external resource) and lives entirely inside a `prefers-reduced-motion: no-preference` media block, so a reduced-motion viewer gets the finished static mosaic; unfurlers run no CSS, so the OpenGraph contract is unchanged. Built from the letter-free analysis bundle (owners, `sequence` cells + seconds), so it spoils nothing (INV-6, `design/post-game/SHARE.md`). An unknown, revoked, or malformed token all resolve to the SAME soft `404` shell, so the surface is no valid-vs-invalid oracle and never confirms token structure. Per-IP rate-limited like the invite unfurl. Additive per section 14 |
| `GET /s/{token}/card.png`             | public                                  | the server-rasterized completion card (`image/png`), built from the SAME analysis bundle and the SAME `@crossy/share-card` builder the web client uses, via resvg with vendored fonts. Two whitelisted query params shape the render: `variant` (`og` default, `1200x630`, the OpenGraph unfurl; or `portrait`, `1080x1620`, the flagship the native clients fetch) and `ground` (`light` default, or `dark` for a dark device). Both default to today's og/light, so a bare `card.png` (what `og:image` emits) is byte-identical to the pre-params render. A `portrait` request for a solo solve renders the builder's solo layout, exactly as the web export does; `og` is never solo. Any unrecognized param value resolves to the SAME soft `404` as an unknown/revoked/malformed token, so the surface is no valid-vs-invalid oracle and names no bad param. Long-cached (`immutable`) for every variant/ground combination: a completed game's card never changes for any shape. Same per-IP rate limit as the shell; no letter enters the render (INV-6). Additive per section 14 |
...
**Invite links.** The shareable invite link is short: `https://{invite-host}/{code}`, carrying only the invite code (the join capability), no `gameId` and no name. This is the canonical form every client emits for sharing (copy, QR, system share); the code alphabet is ASCII-only and CHECK-constrained (`^[2-9A-HJ-NP-Z]{8}$`), so a rendered link cannot carry markup or raw caller input. The invite host serves two audiences from `GET /{code}`: a link unfurler receives a static OpenGraph shell (generic copy, never board state or a solution, DESIGN.md section 7), and a browser is redirected (`302`) to the canonical game view `{web-origin}/game/{gameId}?code={code}`, where the signed-out gate and the existing join flow take over. The redirect resolves the code to its `gameId` by the same normalized ASCII-only lookup as `POST /games/join` (INV-1); a code matching no game lands on the web home, not an oracle `404`, and the redirect exposes no `gameId` the long link did not already carry, since the code is the capability. On iOS the invite host is an associated domain, so an installed app opens `{invite-host}/{code}` directly as a universal link, bypassing the web fallback; the app and every scanner accept a code from any recognized form (a bare code, a link with `?code=`, the legacy `/g/{code}`, and this short `{host}/{code}`), each normalized ASCII-only (INV-1). A signed-out web invite on Apple mobile also offers an explicit open-in-app action using the `crossy://` custom scheme, because a universal link cannot fire from a same-page tap in Safari or the in-app browsers invites travel in (Discord); there is no Smart App Banner until a public App Store id exists (#124). Older links keep resolving unchanged: `/game/{id}?code=` renders the game directly, and `/g/{code}` stays the legacy unfurl. The short form is additive and bumps no version (section 14).

**Share links (`design/post-game/SHARE.md`).** A completed game mints a public completion share link, `{share-origin}/s/{token}`, whose OpenGraph image is the mosaic completion card. The `token` is an unguessable 256-bit URL-safe secret (base64url, `^[A-Za-z0-9_-]{43}$`), so it cannot be guessed and carries no `gameId`; it is the sole capability, and it is minted only by a member of a completed game (`POST /games/{id}/share`, the analysis gate). One active (non-revoked) token exists per game, so the mint is idempotent: a re-share returns the same link rather than a second live one, and revoking a link frees the slot for a fresh mint. The `share-origin` is the config-driven public host, the same `{invite-host}` invite links use, so both families of shareable link live on one host; where the host is unconfigured the origin is the request origin, which also serves `/s`. The share host serves two audiences from `GET /s/{token}`: a link unfurler receives a static OpenGraph shell (`og:title` the puzzle title with the card's fallbacks, `og:image` the `/s/{token}/card.png` PNG at `1200x630`), and a human sees a minimal page whose hero replays the solve: the same mosaic board, drawn inline, painting itself in the bundle's `sequence` order on a fixed loop (pure inline CSS, gated behind `prefers-reduced-motion: no-preference`; a reduced-motion viewer gets the finished static board, and unfurlers, which run no CSS, are untouched). The card is server-rasterized from the SAME letter-free analysis bundle and the SAME `@crossy/share-card` builder the web client uses, so it spoils nothing (INV-6): no board letter is read on any share path. An unknown, revoked, or malformed token all resolve to the SAME soft `404` shell, so the surface confirms nothing about token structure and is no valid-vs-invalid oracle; both routes are per-IP rate-limited like the invite unfurl. The card is long-cached (`immutable`), since a completed game's card never changes. Additive and bumps no version (section 14).
...
The puzzle `title` and `author` are display metadata parsed from the uploaded document at ingestion. They are shown back verbatim and are never normalized or compared, so INV-1 casing does not apply, and they are not solutions, so INV-6 is untouched; ingestion entity-decodes, trims, and caps each at 200 characters, and absent, null, empty, or non-string all read as null. One exception (owner ruling 2026-07-12, #186): a titleless NYT daily is named at ingest from the document's `publicationDate`, formatted "Weekday, Month D, YYYY" (e.g. "Sunday, July 12, 2026") from hardcoded English names, never locale-aware formatting, so the string is deterministic across servers and locales (the INV-1 spirit applied to display metadata). An explicit title always wins over synthesis. The date is parsed strictly (`YYYY-MM-DD`, round-trip validated, so a rolled-over date like `2026-02-30` is rejected); a malformed or absent date falls back to null and the client's "Untitled" rendering. Naming is display metadata and MUST NOT reject the puzzle. They live on the `puzzles` row (not on `ClientPuzzle`/`ServerPuzzle`, which the solve screen does not need), so they surface only where a client shows them back: `title` and `author` on `GET /puzzles`, `title` in the `puzzle` summary on `GET /games`, and both as the additive top-level `puzzleTitle`/`puzzleAuthor` on the `GET /games/{id}` view (the share card's masthead reads them there; the fields sit beside `puzzle`, which stays exactly `ClientPuzzle`).
